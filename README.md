...
게임 업계 근무하면서 그래도 모바일 청첩장은 내가 만들어야지 하는 생각으로 제작해보았습니다. 현 시점에는 이런 개발이 굉장히 허들이 낮은 편이어서 생각보다 수월하게 진행할 수 있었던 것 같습니다.

## 특징

* 용량을 줄이기 위해 Font의 경우 Subset을 사용하였고, 이미지 포맷으로는 Jpeg를 사용했습니다.
* Text의 경우 Mobile Device의 Setting에 맞추어 크기가 변환되도록 하였습니다.
* D-day 카운트 다운 등은 실제 사용하지 않았으나 주석 처리되어 있으니 필요한 경우 사용하시면 될 것 같습니다.
...
다음 기능들은 코드에 구현되어 있으나 실제로는 사용하지 않았습니다. 필요하신 경우 주석을 해제하여 사용하실 수 있습니다.

### D-Day 카운트다운

`src/App.jsx`에 D-Day 카운트다운 기능이 구현되어 있습니다. 결혼식까지 남은 날짜, 시간, 분, 초를 표시하는 타이머입니다.

**활성화 방법:**
1. `src/App.jsx`에서 D-Day 카운트다운 UI를 원하는 위치에 추가
2. `config.wedding.date`와 `config.wedding.time`이 올바르게 설정되어 있는지 확인

### 육행시 (Poem)

`src/config.js`의 `greeting` 섹션에 육행시 기능이 주석처리되어 있습니다. 신랑/신부 이름의 각 글자로 시작하는 시를 표시하는 기능입니다.

**활성화 방법:**
1. `src/config.js`에서 `greeting.poem` 주석 해제
2. 각 줄의 `first`와 `rest` 값을 원하는 내용으로 수정
3. `poemFont`와 `poemSize` 설정 확인
4. `src/App.jsx`에서 육행시 UI를 원하는 위치에 추가
